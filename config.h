...
static Key keys[] = {
	...
	/* Gap controls live on MODKEY|Mod1Mask (Super+Alt), not MODKEY|Mod4Mask:
	 * MODKEY is Mod4Mask here, so MODKEY|Mod4Mask collapses to a bare Super and
	 * every one of these silently shared a combo with a real binding (Super+o
	 * launched the web script AND bumped the outer gaps, Super+6..9 switched tag
	 * AND bumped a gap, ...). keypress() runs every match, not just the first. */
